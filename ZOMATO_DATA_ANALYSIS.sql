...
SELECT A.COUNTRY_NAME, A.REST_COUNT ,ROUND(CAST(A.REST_COUNT AS DECIMAL)/CAST(B.TOTAL_REST AS DECIMAL)*100,2) AS Percent_of_Restaurant
...
ROUND(COUNT(CAST(A.RestaurantID AS DECIMAL))/CAST(B.REST_COUNT AS DECIMAL)*100, 2) as Percent_of_Restaurant
FROM Zomato_Dataset_updated A JOIN COUNTRY_REST B
ON A.COUNTRY_NAME = B.COUNTRY_NAME
...
CT3 AS (
SELECT Locality, Cuisines FROM Zomato_Dataset_updated
)
...
CREATE OR ALTER VIEW VF 
AS
(
SELECT COUNTRY_NAME, City, Locality, N.Cuisines FROM Zomato_Dataset_updated
...
SELECT A.Cuisines, COUNT(A.Cuisines) Total_Cuisines
